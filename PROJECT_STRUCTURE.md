...
本仓库中可能存在 `QDmgt/` 和 `new_task_mgt/` 等历史或外部系统目录：

- `QDmgt/` 是渠道管理系统的参考实现。
- `new_task_mgt/` 是派工管理系统的参考实现。
- 当前 CRM 系统建设只能借鉴这些目录中的业务结构、字段设计、流程拆分和实现思路。
- 当前 CRM 系统不得依赖、导入、调用、修改或混合这两个外部系统的运行代码。
- 新功能必须重构到 `backend/` 和 `frontend/` 的当前系统边界内，并纳入本系统的认证、权限、数据模型、测试和部署流程。

开发、修复和 review 时，除非用户明确要求维护外部系统本身，否则不要把 `QDmgt/` 或 `new_task_mgt/` 纳入改动范围。
...
*本文档基于项目当前状态生成，最后更新时间：2026年4月27日*
