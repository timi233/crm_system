...
本项目的生产建设目标是单一 CRM 销管系统，运行边界为当前 `backend/` FastAPI 后端与 `frontend/` React 前端。

仓库内如存在以下目录，仅作为参考资料使用：

- `QDmgt/`：渠道管理系统参考实现。
- `new_task_mgt/`：派工管理系统参考实现。

这些外部系统可以用于理解渠道管理、派工管理的业务流程、字段结构和交互方式，但不能作为当前 CRM 的依赖、子系统、运行时组件或直接修改目标。相关能力应在当前 CRM 中重新建模和实现，并统一接入 CRM 的用户、权限、审计、测试、构建和部署体系。
...
系统采用基于角色的访问控制（RBAC），各角色的权限范围如下：

| 角色 | 定位 | 读权限 | 写权限 |
|------|------|--------|--------|
| **admin** | 系统管理员 | 全量数据访问 | 全量数据修改 |
| **business** | 业务运营管理员（准管理员） | 全量业务数据访问 | 全量业务实体修改 |
| **finance** | 财务人员 | 财务专用视图（合同、回款计划、项目财务字段） | 财务实体 + owner 检查 |
| **sales** | 销售人员 | owner/channel scope | owner/channel 匹配 |
| **technician** | 技术员 | 工单相关数据 | 仅分配给自己的工单 |

**设计说明**：
- `business` 角色被设计为"准管理员"，可全量访问和修改业务数据，用于业务运营管理场景
- `finance` 角色使用专用财务视图接口 `/customers/{id}/finance-view`，避免访问敏感业务数据
- `sales` 角色只能访问和修改自己负责的或分配给自己的渠道的数据
- `technician` 角色只读，仅可通过工单查看相关数据
