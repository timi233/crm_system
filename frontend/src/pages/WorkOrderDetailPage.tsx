...
const WorkOrderDetailPage = () => {
  ...
  const { data: workOrder, isLoading, isError, error } = useWorkOrder(workOrderId);
  ...
  if (isError) {
    ...
    const errorMsg = axiosError?.response?.data?.detail || axiosError?.message || '加载工单失败';
    const isNotFound = axiosError?.response?.status === 404;
    return (
      <Card>
        <Result
          status={isNotFound ? '404' : 'error'}
          title={isNotFound ? '工单不存在' : '加载失败'}
          subTitle={isNotFound ? '该工单可能已被删除或您无权查看' : errorMsg}
          extra={
            <Button type="primary" icon={<ArrowLeftOutlined />} onClick={() => navigate('/work-orders')}>
              返回工单列表
            </Button>
          }
        />
  ...
  return (
    <Card title={<Space><Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/work-orders')}>返回</Button><span>{workOrder.work_order_no} - {workOrder.customer_name}</span></Space>}
      extra={<Space>
        {workOrder.status !== 'DONE' && workOrder.status !== 'CANCELLED' && workOrder.status !== 'REJECTED' && <><Button type="primary" icon={<CheckCircleOutlined />} onClick={() => nextStatus && handleStatusChange(nextStatus)} disabled={!nextStatus}>{nextStatus ? `流转到${STATUS_LABELS[nextStatus]}` : '已完成'}</Button>{workOrder.status === 'PENDING' && <Button icon={<UserAddOutlined />} onClick={() => setIsAssignModalVisible(true)}>分配服务工程师</Button>}</>}
      ...
      <Descriptions bordered column={2} size="middle">
        ...
        <Descriptions.Item label="服务工程师" span={2}>{workOrder.technician_names && workOrder.technician_names.length > 0 ? <Space>{workOrder.technician_names.map((name, i) => <Tag key={i} color="blue">{name}</Tag>)}</Space> : <span style={{ color: '#999' }}>未分配</span>}</Descriptions.Item>
      ...
      <Drawer
        ...
        onClose={() => { setIsSummaryModalVisible(false); setServiceSummary(''); setPendingStatus(null); }}
        width={520}
        maskClosable={false}
        destroyOnClose
      >
        ...
        />
        <Button type="primary" onClick={handleSummarySubmit} loading={updateStatusMutation.isPending} block>
          保存
        </Button>
      </Drawer>
      ...
      <Drawer
        ...
        onClose={() => { setIsAssignModalVisible(false); assignForm.resetFields(); }}
        width={520}
        maskClosable={false}
        destroyOnClose
      >
        <Form form={assignForm} layout="vertical">
          <Form.Item name="technician_ids" label="选择服务工程师" rules={[{ required: true, message: '请至少选择一名服务工程师!' }]}>
            <Select mode="multiple" placeholder="请选择服务工程师" showSearch optionFilterProp="children">
              {technicianOptions.map(opt => <Option key={opt.value} value={opt.value}>{opt.label}</Option>)}
            </Select>
          </Form.Item>
        </Form>
        <Button type="primary" onClick={handleAssignTechnicians} loading={assignTechniciansMutation.isPending} block>
          保存
        </Button>
      </Drawer>
      <Drawer
        ...
        onClose={() => { setIsEvaluationModalVisible(false); evaluationForm.resetFields(); }}
        width={520}
        maskClosable={false}
        destroyOnClose
      >
        <Form form={evaluationForm} layout="vertical">
          <Form.Item name="quality_rating" label="服务质量" rules={[{ required: true, message: '请评分!' }]}>
            <Rate />
          </Form.Item>
          <Form.Item name="response_rating" label="响应速度" rules={[{ required: true, message: '请评分!' }]}>
            <Rate />
          </Form.Item>
          <Form.Item name="customer_feedback" label="客户反馈">
            <TextArea rows={3} placeholder="请描述您的服务体验" />
          </Form.Item>
          <Form.Item name="improvement_suggestion" label="改进建议">
            <TextArea rows={2} placeholder="帮助我们提供更好服务的建议" />
          </Form.Item>
          <Form.Item name="recommend" label="是否推荐" rules={[{ required: true, message: '请选择是否推荐!' }]}>
            <Select>
              <Option value={true}>会推荐</Option>
              <Option value={false}>不会推荐</Option>
            </Select>
          </Form.Item>
          <Button type="primary" onClick={handleCreateEvaluation} loading={createEvaluationMutation.isPending} block>
            提交评价
          </Button>
        </Form>
      </Drawer>
