...
async def get_contract(
    ...
    result = await db.execute(
        ...
        .options(
            selectinload(Contract.products),
            selectinload(Contract.payment_plans),
            selectinload(Contract.project),
            selectinload(Contract.terminal_customer),
        )
    )
...
async def update_contract(
    ...
    if not existing:
        raise HTTPException(status_code=404, detail="Contract not found")
    await db.refresh(existing, ["project", "terminal_customer"])
    await policy_service.authorize(
        resource="contract",
        action="update",
        principal=principal,
...
async def delete_contract(
    ...
    if not contract:
        raise HTTPException(status_code=404, detail="Contract not found")
    await db.refresh(contract, ["project", "terminal_customer"])
    await policy_service.authorize(
        resource="contract",
        action="delete",
        principal=principal,
